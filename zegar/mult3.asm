.386P
WIELKOSC_ELEMENTU EQU 4		;Rozmiar pola danych w elemencie kolejki.
...
;Dodanie zadania g³ównego do kolejki zadañ:
  MOV AX, 0          	;Priorytet aktualny 0, priorytet  0.
...
PRZERWANIE0 PROC
  ...
  MOV AL, [ES:SI+2]    
  MOV [ES:SI+3], AL   	;Przywrócenie priorytetu 
;Rotowana jest kolejka zadañ a¿ do uzyskania na pierwszej pozycji zadania z polem priorytetu aktualnego równym 0:
  PETLA_WYBORU_PRIORYTETU0:  
    CALL PRZELACZ_ZADANIA   
    MOV AL, [ES:SI+3]   
    CMP AL, 0   
    JE WYJSCIE_Z_PETLI_WYBORU_PRIORYTETU0 
    DEC BYTE PTR [ES:SI+3]	;Dekrementacja bajtu przechowuj¹cego priorytet aktualny.
    JMP PETLA_WYBORU_PRIORYTETU0 
  WYJSCIE_Z_PETLI_WYBORU_PRIORYTETU0:  
;Rejestr DI zawiera selektor TSS zadania ustawionego na pierwszej pozycji kolejki zadañ dziêki zastosowaniu 
;procedury  PRZELACZ_ZADANIA, natomiast CX zawiera selektor  TSS  obecnego zadania (zwrócony
;z procedury  ZWROC_SELEKTOR_TSS_AKTUALNEGO_ZADANIA).
;Je¿eli oba te selektory s¹ identyczne (na przyk³ad gdy jest tylko jedno zadanie) to nie mo¿na pozwoliæ 
;na dalsze wykonywanie siê procedury - w Intelowskiej architekturze rekursywne wywo³ywanie siê zadañ 
;jest niedozwolone.
  CMP CX, DI 
...
;Jako, ¿e segment stosu jest dla wszystkich zadañ jednakowy, nale¿y ustawiæ im ró¿ne szczyty stosu. 
